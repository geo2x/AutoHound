...
from autohound.constants import MAX_HVT_IN_REPORT
...
"""
        ...
        da_count = len([p for p in self.attack_paths if 'Domain Admin' in str(p.end_node)])
        summary = f"""## Executive Summary

AutoHound identified **{len(self.attack_paths)} attack path(s)** in the analyzed Active \
...
- **Total Paths to Domain Admin:** {da_count}
...
"""
    ...
    def _list_high_value_targets(self, hvt: list) -> str:
        ...
        lines = []
        for node in hvt[:MAX_HVT_IN_REPORT]:  # Limit for report readability
            flags = []
        ...
        if len(hvt) > MAX_HVT_IN_REPORT:
            lines.append(f"\n_... and {len(hvt) - MAX_HVT_IN_REPORT} more high-value targets_")
    ...
    def _generate_path_detail(self, number: int, path: AttackPath) -> str:
        ...
        for step in path.steps:
            ...
            sections.append(
                f"**Relationship:** `{step.source_node}` --[{step.edge_type.value}]--> "
                f"`{step.target_node}`\n"
            )
            ...
            if step.attack_technique_id:
                sections.append(
                    f"**MITRE ATT&CK:** [{step.attack_technique_id}]"
                    f"(https://attack.mitre.org/techniques/{step.attack_technique_id}/) - "
                    f"{step.attack_technique_name}"
                )
