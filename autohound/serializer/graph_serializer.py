...
from autohound.constants import (
    DEFAULT_MAX_CHUNK_SIZE,
    MAX_ALL_NODES,
    MAX_EDGE_DESCRIPTIONS,
    MAX_HVT_IN_REPORT,
    MAX_INBOUND_EDGES_DISPLAY,
    MAX_PRIORITY_NODES,
)
...
class GraphSerializer:
    ...
    def __init__(self, graph: Graph, max_chunk_size: int = DEFAULT_MAX_CHUNK_SIZE):
        """
...
**Analysis Objective:** Identify novel attack paths from low-privilege starting points to \
Domain Admin or equivalent compromise.
"""
        return summary
    ...
    def _describe_high_value_targets(self) -> str:
        ...
        desc.append(
            "These are Tier 0 assets - compromise of any grants significant or complete "
            "domain control.\n"
        )
        ...
        for node in hvt:
            ...
            if inbound:
                ...
                for edge in inbound[:MAX_INBOUND_EDGES_DISPLAY]:  # Limit for readability
    ...
    def _describe_prioritized_nodes(self) -> str:
        ...
        for node in priority_nodes[:MAX_PRIORITY_NODES]:  # Limit for context
            if node is None:
                continue
            desc.append(f"\n## {node.name}")
            desc.append(f"- Type: {node.node_type.value}")
    ...
    def _describe_all_nodes(self) -> str:
        ...
        for node in list(self.graph.nodes.values())[:MAX_ALL_NODES]:  # Hard limit
            desc.append(f"\n- {node.name} ({node.node_type.value})")

        if self.graph.node_count() > MAX_ALL_NODES:
            desc.append(f"\n... and {self.graph.node_count() - MAX_ALL_NODES} more nodes")
    ...
    def _describe_relationships(self) -> str:
        ...
        if dangerous_edges:
            ...
            for edge in dangerous_edges[:MAX_HVT_IN_REPORT]:
                source = self.graph.get_node(edge.source_id)
                target = self.graph.get_node(edge.target_id)
    ...
    def _describe_edge_list(self, edges: list[Edge], title: str) -> str:
        ...
        for edge in edges[:MAX_EDGE_DESCRIPTIONS]:  # Limit
            source = self.graph.get_node(edge.source_id)
            target = self.graph.get_node(edge.target_id)
        ...
        if len(edges) > MAX_EDGE_DESCRIPTIONS:
            desc.append(f"\n... and {len(edges) - MAX_EDGE_DESCRIPTIONS} more relationships")
